...
ModuleError::ModuleError(string module_path, string module_name, 
			 string interface, string msg) 
{
...
void *module_create_instance(string module_path, string module_name, 
			     string interface_name) throw (ModuleError) 
{
	...
	if (!g_module_symbol(module, instance_fkt_name.c_str(), 
	    (gpointer*) &module_instance_function )) {
		throw ModuleError(module_path, module_name, interface_name, 
				  "Instance function " + instance_fkt_name  + 
				  "() not implemented. Are you sure this module offers the interface you are looking for?");	 
	}
	...
	throw ModuleError(module_path, module_name, interface_name, 
			  "Instance function returned a null pointer.");
}
