...
siox nutzt die übergeben Daten, um einen neuen IOPm-Knoten zu erzeugen und – wenn möglich – in ggf. existierende Graphen einzubinden. Die Parameter stehen für InstanzID (je nach Knotenart, z.B. die ProzeßID eines Threads oder der IP-Adresse und Port eines Servercaches), SoftwareID (z.B. „MPI“ oder „POSIX“) und HardwareID (z.B. „Blizzard Node 5“ oder die MacID einer NAS-Festplatte). Der Knoten kann SWID und sogar HWID bereitstellen. Andernfalls bestimmt siox sie selbst.
...
Ruft ein Knoten später siox_register() auf, versucht ihn siox anhand dieser Informationen in den vorhandenen Graphen einzupflegen.
 */
...
/**
 * Der Knoten beendet – soweit es siox betrifft – seine Arbeit und stößt damit abschließende Speicher- und Analysearbeiten an.
Deskriptoren
 */
...
Die Funktion ist für jeden Deskriptor, der nicht von einem anderen abgeleitet oder von einem anderen Knoten empfangen wird, aufzurufen.
...
Was ist, wenn der Deskriptor zur Zuordnung etwaiger Folgeaktionen auf niedrigerer Ebene noch benötigt werden sollte? Können wir diese in allen Fällen über die Zeitstempel korrekt zuordnen? Selbst bei Write-Behind-Caching? Selbst, wenn der Deskriptor im selben Knoten erneut vergeben wurde?
Aktivitäten
 */
...
Beispiel: Das Senden einer Anzahl Bytes an einen übergeordneten Knoten, die über ein Dateihandle angefordert wurden.
...
Sind außer den Zeitstempeln keine weiteren Leistungsdaten zu melden, kann die Funktion auch direkt statt siox_stop_activity() aufgerufen werden.
 */
...
Beispiel: Die maximale Prozessorauslastung der vom Knoten repräsentierten Komponente während der letzten Sekunde.

Wann wird was gemeldet? Woher weiß die Komponente, ob – und wenn ja, auf was – wir die Leistungsdaten beschränken wollen?
 */
