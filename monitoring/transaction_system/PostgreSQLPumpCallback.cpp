...
PostgreSQLPumpCallback::PostgreSQLPumpCallback(PGconn &dbconn) : dbconn_(&dbconn)
{
}


void PostgreSQLPumpCallback::store_activity(Activity &act) const
{

}


uint64_t PostgreSQLPumpCallback::insert_activity_id(const ActivityID &aid) const
{
	...
	const Oid param_types[nparams] =   {20, 20, 20, 20, 20, 20};
	const int param_formats[nparams] = {1,   1,  1,  1,  1,  1};
	const int result_format = 0;

	const char *command =
	    "INSERT INTO activities.activity_id (                              \
			id, thread_id,                                         \
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		...
		logger->log(Logger::ERR, "DB insert failed: %s.", PQresultErrorMessage(res));
...
#ifndef NDEBUG
	logger->log(Logger::NOTICE, "Successfully inserted activity id.");
#endif
	PQclear(res);
...
// 
// 
// void PostgreSQLPumpCallback::store_component() const
// {
// 	uint32_t iid = insert_interface();
// 	
// }
// 
// 
...
// 
// 
// void PostgreSQLPumpCallback::store_node(const std::string &hwdesc, const std::string &swdesc, const std::string &indesc) const
// {
...
// 			RETURNING id;";
// 
// 	PGresult *res = PQexecParams(dbconn_, command, nparams, param_types, param_values, param_lengths, param_formats, result_format);
// 
// 	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
// 
...
// 
// 
// uint64_t PostgreSQLPumpCallback::insert_node(uint64_t hwid, uint64_t swid, uint64_t iid) const
// {
// #ifndef NDEBUG
...
// 
// 
// std::string PostgreSQLPumpCallback::serialize_attributes(const Activity &act) const
// {
...
// 
// 
// void PostgreSQLPumpCallback::insert_parent(const ActivityID &parent, uint64_t child_id) const
// {
