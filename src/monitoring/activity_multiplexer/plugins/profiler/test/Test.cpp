...
int main( int argc, char const * argv[] )
{
	...
	ActivityProfilerPlugin * ap = core::module_create_instance<ActivityProfilerPlugin>( "", "siox-monitoring-activityPlugin-ActivityProfiler", ACTIVITY_MULTIPLEXER_PLUGIN_INTERFACE );

	// init plugin
	// not necessary, but for testing...
	ActivityProfilerPluginOptions & op = ( ActivityProfilerPluginOptions & ) ap->getOptions();
	op.multiplexer = m1;
	...
	ComponentReport cr = dynamic_cast<ComponentReportInterface*>(ap)->prepareReport();
