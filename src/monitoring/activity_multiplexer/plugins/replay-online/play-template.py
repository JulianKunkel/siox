...
'register_attribute': {
	...
	'player_attribute_macros': '''
	#define SUB_%(AttributeVariable)s "%(Domain)s", "%(Name)s"
	#define SUB_TYPE_%(AttributeVariable)s SUB_TYPE_%(StorageType)s 
	#define SUB_CAST_%(AttributeVariable)s SUB_CAST_%(StorageType)s 
	''',
...
'activity_attribute': {
    ...
    #'player': '''SUBSTITUE_TYPE_%(Attribute)s %(Value)s = getActivityAttributeValueByName(activity, SUBSTITUTE_%(Attribute)s).SUBSTITUE_CAST_%(Attribute)s();''',
    'player': '''%(Value)s = getActivityAttributeValueByName(activity, SUB_%(Attribute)s).SUB_CAST_%(Attribute)s();''',
},
...
siox_activity_get_attribute( %(Activity)s, %(Attribute)s, &%(Value)s );
    """,
    'player_after': """
			Attribute attr( oa_%(Attribute)s.aID, convert_attribute( oa_%(Attribute)s, &%(Value)s ) );               
...
"globalOnce": "",
# Will be included
"includes" : ['<stdlib.h>', '<stdio.h>', '<stdarg.h>', '<assert.h>', '<string.h>', '<unistd.h>']
}
