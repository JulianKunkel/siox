...
{
	// init the activity multiplexer
	ActivityMultiplexer * m1 = core::module_create_instance<ActivityMultiplexer>( 
		"",
		"siox-monitoring-ActivityMultiplexerAsync", 
		"monitoring_activitymultiplexer"
	);

	// register the replay plugin
	ActivityMultiplexerPlugin * ap = core::module_create_instance<ActivityMultiplexerPlugin>( 
		"",
		"siox-monitoring-activityPlugin-Replay",
		ACTIVITY_MULTIPLEXER_PLUGIN_INTERFACE
	);

	// init plugin
	// not necessary, but for testing...
	ReplayPluginOptions & op = ( ReplayPluginOptions & ) ap->getOptions();
