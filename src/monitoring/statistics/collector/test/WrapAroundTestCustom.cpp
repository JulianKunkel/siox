/**
 * @file ./src/monitoring/statistics/collector/test/WrapAroundTestCustom.cpp
 *
...
const uint64_t kMin = 23;
const uint64_t kMax = 48;
const uint64_t kStart = 29;
const uint64_t kIncrement = 10;

class ProviderDummy : public StatisticsProviderPlugin {
	public:
		ProviderDummy() {
			metric = (uint64_t)kStart;
			cout << "ProviderDummy: Setting metric to " << metric << "\n";
		}
		virtual void nextTimestep() throw() {
			metric += kIncrement;
			if(metric > kMax) metric += -(kMax + 1 - kMin);
			cout << "ProviderDummy: Setting metric to " << metric << "\n";
		}
		virtual vector<StatisticsProviderDatatypes> availableMetrics() throw() {
			vector<StatisticsProviderDatatypes> result;
			result.push_back(StatisticsProviderDatatypes("foo", "@localhost", metric, INCREMENTAL, "bar", "baz", kMax, kMin));
			return result;
		}
	private:
	...
	public:
		...
		virtual void newDataAvailable() throw() override {
			if(myStatistic) {
				...
				if(gotFirstValue) {
					assert(myStatistic->curValue == kIncrement);
				}
				gotFirstValue = true;
			}
		}
