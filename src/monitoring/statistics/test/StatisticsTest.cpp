...
//		StatisticsDescription description( ontology->lookup_attribute_by_name( "Statistics", "test/weather" ).aID, {{"node", LOCAL_HOSTNAME}, {"tschaka", "test2"}} );
		StatisticsDescription description( ontology->lookup_attribute_by_name( "Statistics", "test/weather" ).aID, topology->lookupObjectByPath( TOPO_ALIAS_SEP "localhost" TOPO_PATH_SEP "weather-station:0" ).id() );
