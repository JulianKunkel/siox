 ...
 *  @file gap_buffer.h
 *
 *  @Author: Marc Wiedemann - Hsin Tsao
 *  @Version: 1.2 2013
 *
...
GapBuffer::GapBuffer(const GapBuffer& tb)
{
     ...
     buffer = (wchar_t *) malloc(tb.bufend - tb.buffer);
     
     //wchar_t strcpy(buffer, tb.buffer);
 ...
 */
int GapBuffer::CopyBytes(wchar_t *destination, wchar_t *source, unsigned int length)
{
...
{   
    ...
    if ( ( (bufend - buffer) + size) > BufferSize() ) {
        ...
        wchar_t *origbuffer = buffer;

        int NewBufferSize = (bufend - buffer) + size  + GAP_SIZE;
        
        buffer = (wchar_t *) realloc(buffer, NewBufferSize);
 ...
 */
wchar_t GapBuffer::PreviousChar()
{
 ...
 */
void GapBuffer::ReplaceChar(wchar_t ch)
{
...
void GapBuffer::PutChar(wchar_t ch)
{
    InsertChar(ch);
    *point++;
 ...
 */
void GapBuffer::InsertChar(wchar_t ch)
{
...
void GapBuffer::InsertString(wchar_t *string, unsigned int length)
{
...
/*
 * Here we initialize the buffer and set
 * the pointers to the correct position.
 */
int GapBuffer::InitBffr(unsigned int size)
{
...
int main( int argc, char* argv[] )
{
    GapBuffer* b;
//UTF-8 compatible 
    wchar_t dest[64] = L"Component:\\Activity";
    const wchar_t * src = L"\\Component-NIC-Model-00001";
    wchar_t * ptr;
